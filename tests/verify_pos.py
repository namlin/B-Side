...
def home_page_verification():
    """Test Home page serving"""
...
def stock_verification():
    """Test Consult Available Inventory"""
    ...
    if status == 200 and isinstance(products, list):
        ...
        for p in products:
            if p["id"] == 1:
                print(f"        Product 1: '{p['name']}' by '{p['artist_name']}' - \
                      Price: ${p['price']} - Stock: {p['stock']} units.")
            if p["id"] == 4:
                print(f"        Product 4: '{p['name']}' by '{p['artist_name']}' - \
                      Price: ${p['price']} - Stock: {p['stock']} units.")
    else:
    ...
    cart_payload = {
        "items": [
            ...
            {"product_id": 4, "quantity": 1}  # 1 x $35.00
        ]
    }
    ...
    if status == 201:
        ...
        print(f"        Sale Summary: ID: {result['sale']['sale_id']} - Total: \
              ${result['sale']['total']} - Items: {result['sale']['items_count']}")
        # Expected total: 2*25.00 + 1*35.00 = 85.00
        if float(result['sale']['total']) == 85.00:
            print("        [OK] Sale total calculated correctly ($85.00).")
        else:
    ...
    status, products = make_request("/api/products")
    if status != 200:
        print(f"   [FAIL] Checkout failed with status {status}")
...
def insuficient_stock_verification():
    """Test Checkout with insufficient stock (Validation rejection)"""
...
def transaction_rollback_verification():
    """Test Transaction atomic rollback verification"""
    ...
    if p1_final_stock == p1_initial_stock:
        print(f"   [OK] Transaction rolled back atomically. Stock of Product 1 remains at \
              {p1_final_stock}.")
    else:
        print(f"   [FAIL] Transaction atomicity failed. Stock of Product 1 decreased from \
              {p1_initial_stock} to {p1_final_stock}!")

def run_verification():
    """Performs automated tests to verify the functionality of the API system"""
    ...
    insuficient_stock_verification()
